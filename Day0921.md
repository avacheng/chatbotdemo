...
* * * 
如果使用 [EchoBot.cs完整程式碼](https://github.com/avacheng/chatbotdemo/blob/master/EchoBot.cs) ：修改 namespace 的**專案名稱**
```
...
// 建立將「question」文字傳入後，會得到 QnA 答案的方法 (透過 WebAPI 和 QnA maker 溝通)
...
附註：從 BotChannel 傳來的 json 資料格式 ( 用 turnContext.Activity.xxx 取得項目值 )<br>
```
...
* * *
[Json Parser](http://json.parser.online.fr/)<br>
附註：QnA maker 給的 json 回答 ( 用 JObject.Parse 取得項目值 ) <br>
```
...
* * *
登入 [LUIS](https://www.luis.ai/home) (第一次登入後需要進行資料註冊)<br>
